...
#ifndef _HEADER_FILE_CFD_CF_UTILITY_FILEPATH_HPP_
#define _HEADER_FILE_CFD_CF_UTILITY_FILEPATH_HPP_
...
// /temp/a  --> don't create
inline cf_int CreateDirs(cf_cpstr dir,cf_uint32 len)
{
    if(len<2)
        return 0;
    cf_char dirname[2048];
    strncpy(dirname,dir,sizeof(dirname));
    for(cf_uint32 i=1; i<len; i++)
    {
        if(dirname[i]=='/')
        {
            dirname[i] =0;
...
{
public:
    PidFileWriter(cf_cpstr filePathName):_filePathName(filePathName)
    {
    }
    cf_void SetFilePathName(cf_cpstr filePathName)
    {
        _filePathName.assign(filePathName,strlen(filePathName));
    }
    cf_void Set()
    {
        WritePid();
...
#endif // _HEADER_FILE_CFD_CF_UTILITY_FILEPATH_HPP_
